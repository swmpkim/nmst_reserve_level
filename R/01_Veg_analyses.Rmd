...
# library(khroma) # for okabeito palette, color-blind-friendly
...
# for troubleshooting: find out if any species in the data frame aren't in the species list
# spps_live2 <- spps_live[which(spps_live %in% names(dat))]
# names(dat)[which(!(names(dat) %in% spps_live2)[1:50])]
...
# palcols_top <- khroma::color("okabeito")(4)
...
# palcols_spec <- khroma::color("okabeito")(n)
...
# palcols_univar <- khroma::color("okabeito")(n)
...
# if(n <= 8) palcols_multivar <- khroma::color("okabeito")(n)
# if(n == 9) palcols_multivar <- c(khroma::color("okabeito")(8), "#BBBBBB")
