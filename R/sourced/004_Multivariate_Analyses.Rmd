...
for(i in seq_along(zones)){
    ...
    if(pmvt$`Pr(>F)`[row.names(pmvt) == "Time_group"] < 0.05){
        ...
        names(simp_smaller) <- c("average contribution", "sd", "cumulative", "p")
...
for(i in seq_along(simps)){
    print(kable(x = simps[[i]], 
                caption = paste0("Vegetation Zone ", names(simps)[i], " - 6 species contributing most to differences between first and most recent years of monitoring"),
                digits = 4,
                format.args = list(scientific = FALSE)) %>% 
              kable_styling("striped")
          )
