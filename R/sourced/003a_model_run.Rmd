...
```{r}
mod_nested_full <- lmer(EIR ~ Years_sinceStart*Vegetation_Zone + (1|SiteID/TransectID/PlotID),
                   data = dat)

mod_nested_trns <- lmer(EIR ~ Years_sinceStart*Vegetation_Zone + (1|StTrns/PlotID),
                   data = dat)

mod_plts <- lmer(EIR ~ Years_sinceStart*Vegetation_Zone + (1|StTrnsPlt),
                   data = dat)
```
...
if(exists("mod_nested_trns")){
    ...
    p2 <- ggplot() +
        geom_jitter(data = dat, aes(x = Years_sinceStart, y = EIR,
                                     col = Vegetation_Zone),
                    alpha = 0.7) +
        geom_line(data = pdf, aes(x = Years_sinceStart,
                                  y = yvar,
