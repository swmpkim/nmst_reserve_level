...
```{r source-003a-userChosen1, child = "003a_model_run.Rmd"}
...
```{r source-003a-userChosen2, child = "003a_model_run.Rmd"}
...
```{r source-003a-userChosen3, child = "003a_model_run.Rmd"}
...
```{r source-003a-userChosen4, child = "003a_model_run.Rmd"}

```


```{r export-univar-zones}
# export univar results
all_univars <- bind_rows(univar_outputs, .id = "ResponseVariable") %>% 
    mutate(Reserve = params$file_code) %>% 
    rename(Change_perYear = Years_sinceStart.trend) %>% 
    relocate(Reserve)
filename <- paste0(params$file_code, "_UnivarTrends_", Sys.Date(), ".csv")
write.csv(all_univars, file = here::here("output", filename), row.names = FALSE)

# export zone information for use in exploring trend outputs
...
zone_tbl <- stn_tbl %>% 
    group_by(Reserve, Vegetation_Zone) %>% 
    summarize(across(c(Lat, Long), function(x) mean(x, na.rm = TRUE)),
              across(c(Orthometric_Height, Height_Relative_to_MLLW),
                     summ_stats))
filename <- paste0(params$file_code, "_ZoneInfo_", Sys.Date(), ".csv")
write.csv(zone_tbl, file = here::here("output", filename), row.names = FALSE)
```
