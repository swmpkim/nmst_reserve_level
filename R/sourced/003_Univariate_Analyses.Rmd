...
# limit these to "if at least one metric is specified" .......

# calculate all necessary columns
# based on original 'dat'

# first custom metric
if(!is.na(anaSpecs$Choice[anaSpecs$R_varName == "custom1"])){
...
if(!is.na(anaSpecs$Choice[anaSpecs$R_varName == "custom2"])){
...
if(exists("dat_custom")){
    ...
    dat_custom <- dat_custom %>% 
        mutate(Custom_metric1 = !!user_formula1_parsed)
    
    # custom metric 2 may or may not exist, so make it an if statement
    if(!is.na(anaSpecs$Choice[anaSpecs$R_varName == "custom2"])){
        dat_custom <- dat_custom %>% 
            mutate(Custom_metric2 = !!user_formula2_parsed)
    }
    # select proper columns
    dat_custom <- dat_custom %>% 
        select(Reserve:Total,
               starts_with("Custom_metric"))
