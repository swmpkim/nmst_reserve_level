...
```{r}
# for multivar, also remove any plots that don't have readings in the first couple years?? how to deal with this though when there are only some early plots, but most of what a reserve is interested in started later?

# maybe collapse to start/end by plot???
```
...
```{r}
# remove any plots that don't have >3 readings
...
names(palcols_top) <- unique(top_3_mean$Species)
...
names(palcols_spec) <- unique(anaSpec_dfs$spat$Species_or_Group)
...
names(palcols_univar) <- unique(anaSpec_dfs$univar$Species_or_Group)
```
