...
+  get rid of density and height columns  
...
```{r}
dat2 <- dat %>% 
    select(-starts_with("Average Canopy Height"),
...
to_find_empties <- dat2 %>% 
    select(-(Reserve:Notes),
           -starts_with("F"))

# from code behind janitor::remove_empty; line 12
empty_rows <- which(rowSums(is.na(to_find_empties)) == ncol(to_find_empties))

no_nas <- dat2[-empty_rows, ]
```
