...
## Check for models that didn't run   
...
of COURSE they already wrote a function for this. If TRUE prints out, the model is singular.  
```{r}
if(exists("mod_nested_lme4")){
    isSingular(mod_nested_lme4)
}
```
...
Can use VarCorr on nlme or lme4 models to find the problematic variance components  

```{r}
VarCorr(mod_nested_nlme)
VarCorr(mod_nested_lme4)
```
